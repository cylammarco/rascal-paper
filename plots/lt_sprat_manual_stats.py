...
c.set_hough_properties(num_slopes=2000,
                       ...
                       min_wavelength=3500.,
                       max_wavelength=8000.)
c.set_ransac_properties(sample_size=5, top_n_candidate=6, filter_close=True)
...
for mt in max_tries:
    ...
    for i in range(N):
        ...
        print('max_tries: {}, repetition: {} of {}'.format(mt, i + 1, N))
...
np.save(os.path.join(base_dir, 'sprat_matched_peaks_manual_mt'),
        matched_peaks_mt)
np.save(os.path.join(base_dir, 'sprat_matched_atlas_manual_mt'),
        matched_atlas_mt)
...
np.save(os.path.join(base_dir, 'sprat_peak_utilisation_manual_mt'),
        peak_utilisation_mt)
np.save(os.path.join(base_dir, 'sprat_atlas_utilisation_manual_mt'),
        atlas_utilisation_mt)
